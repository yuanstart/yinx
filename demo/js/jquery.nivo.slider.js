		...
		var h = function(b) {
			...
			if (e.currentImage.attr("title") != "" && e.currentImage.attr("title") != undefined) {
				...
				if (c.css("display") == "block") {
					c.find("p").stop().fadeOut(b.animSpeed,
					function() {
						a(this).html(d);
			...
			for (var k = 0; k < g.length; k++) {
				if (d.controlNavThumbs) {
					...
					if (d.controlNavThumbsFromRel) {
						j.append('<a class="nivo-control" rel="' + k + '">1</a>')
					} else {
						j.append('<a class="nivo-control" rel="' + k + '">2</a>')
					}
				} else {
					j.append('<a class="nivo-control" rel="' + k + '">●</a>')
				}
			}
			a(".nivo-controlNav a:eq(" + e.currentSlide + ")", f).addClass("active");
			a(".nivo-controlNav a", f).on("click",
			function() {
				if (e.running) return false;
