...
class UploadFile{
	...
	public function upLoad($file,$save_folder,$file_name,$allow_types='jpg|gif|png|zip|rar|txt|html|bmp|pdf',$max_size=2000000000,$overfile=false){
		...
		//转编码
		if ($this->is_utf8($this->sava_path)){
			$gbkpath=iconv('utf-8','gb2312',$this->sava_path);
		}else{
			$gbkpath=$this->sava_path;
		}
		if ($this->is_utf8($this->file_name)){
			$gbkname=iconv('utf-8','gb2312',$this->file_name);
		}else{
			$gbkname=$this->file_name;
		}
		//得到gbk编码的路径
		$new_name=$gbkpath.$gbkname;
	 ...
	 *生成经销商证书
	 *$file原图片文件名--upimg/d20150110101836.jpg
	 *$save_folder保存的路径--upimg
	 *$file_name保存的文件名--d20150110101836
	 *$arr位置和文字数组
	 *$ttf文字水印所用的字体文件路径--arial.ttf
	 */
	public function makeage($file,$save_folder,$file_name,$arr){
		$sfile=MO_ROOT.'/'.$file; //模板路径
		$this->sava_path=MO_ROOT.'/'.$save_folder.'/'; //保存的目录
		$dfile=$this->sava_path.$file_name.'.'.$this->fileext($file);//保存的路径
		if (file_exists($sfile)){
			if ($info=getimagesize($sfile)){
				$swidth=$info[0];
				$sheight=$info[1];
				...
				if($sim=$this->getfileim($sfile,$stype)){
					...
					//设置两个颜色是想字体更好看，目前只用了一种
					//$black = imagecolorallocatealpha($sim, 0,0,0,100);
					$white = imagecolorallocatealpha($sim, 255,255,255,0);
					//循环把文字写入图片对象
					foreach($arr as $v){
						...
						imagettftext($sim,$v['s'],0,$textxx,$textxy,$white,MO_ROOT.'/'.$v['f'],$v['t']);	
					}
					//创建目录
					if(!$this->setSavaPath()){
						$this->error=8;
					...
					//生成图片
					if($this->createfileim($sim,$stype,$dfile,95)){
						imagedestroy($sim);
						return $save_folder.'/'.$file_name.'.'.$this->fileext($file);
					}else{
						$this->error=15;
						return false;
					}
