...
if ($id==''||!checknum($id)||$lm==''||$title==''||$px==''||!checknum($px)){
	msg('参数错误');
}

//判断经销商名是否重复
if ($title!=''){
	$rs=$db->getrs('select title from '.table($conf['sy']['table_co']).' where title="'.$title.'" and id<>'.$id.'');
	if ($rs){
...
if ($conf['co']['ageno']==true&&$ageno!=''){
	$rs=$db->getrs('select title from '.table($conf['sy']['table_co']).' where ageno="'.$ageno.'" and id<>'.$id.'');
	if ($rs){
...
if ($conf['co']['wx']==true&&$wx!=''){
	$rs=$db->getrs('select title from '.table($conf['sy']['table_co']).' where wx="'.$wx.'" and id<>'.$id.'');
	if ($rs){
...
if ($conf['co']['phone']==true&&$phone!=''){
	$rs=$db->getrs('select title from '.table($conf['sy']['table_co']).' where phone="'.$phone.'" and id<>'.$id.'');
	if ($rs){
		msg('存在相同的电话号码，使用公司名为：'.$rs['title']);
	}
}

$sql='update '.table($conf['sy']['table_co']).' set `lm`="'.$lm.'",`title`="'.$title.'",`ageno`="'.$ageno.'",`area`="'.$area.'",`idcard`="'.$idcard.'",`wx`="'.$wx.'",`qq`="'.$qq.'",`phone`="'.$phone.'",`stime`="'.$stime.'",`etime`="'.$etime.'",`f_body`="'.$f_body.'",`z_body`="'.$z_body.'",`img_sl`="'.$img_sl.'",`ym_tit`="'.$ym_tit.'",`ym_key`="'.$ym_key.'",`ym_des`="'.$ym_des.'",`px`='.$px.',`wtime`='.$wtime.' where `id`='.$id.'';
$db->execute($sql);
