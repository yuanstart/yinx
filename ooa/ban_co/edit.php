...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border_tab">
  ...
  ?>
  <tr class="tdbg">
    <td width="120" align="right"><?php echo $v['name']?>连接地址：</td>
    <td><INPUT name="link_url<?php echo $v['lang']?>" type="text" id="link_url<?php echo $v['lang']?>" size="57" maxlength="250" value="<?php echo $row['link_url'.$v['lang']]?>"></td>
  </tr>
  <?php
		if ($conf['co']['mlang']!=true||$conf['co']['link_url_lang']==true){
			break;
		}
  	}
  }
  ...
  ?>
  <tr class="tdbg" >          
    <td width="120" align="right"><?php echo $v['name']?>图片上传：</td>          
    <td >
    <iframe name="frame1<?php echo $v['lang']?>" id="frame1<?php echo $v['lang']?>" src="up_image_tool/uploadd.php?frameid=frame1<?php echo $v['lang']?>&kuang=img_sl<?php echo $v['lang']?>&img_sl=<?php echo $row['img_sl'.$v['lang']]?>" style="margin-top:2px; width:auto; width:380px; height:22px; overflow:hidden;" frameborder="0"  scrolling="no"></iframe>  			    
    <input type="hidden" name="img_sl<?php echo $v['lang']?>" id="img_sl<?php echo $v['lang']?>" value="<?php echo $row['img_sl'.$v['lang']]?>" />
    <?php
