...
if (read_config('config')){
	//获取系统配置
	$sy_id=$conf['sy']['id']=(!empty($_POST['id']))?(int)html($_POST['id']):10;
	$conf['sy']['pre']=(!empty($_POST['pre']))?html($_POST['pre']):'book';
	$conf['sy']['name']=(!empty($_POST['name']))?html($_POST['name']):'留言';
	$conf['sy']['table_co']=(!empty($_POST['table_co']))?html($_POST['table_co']):'book';
	$conf['sy']['huifu']=(!empty($_POST['huifu']))?changety(html($_POST['huifu'])):true;
	$conf['sy']['send']=(!empty($_POST['send']))?changety(html($_POST['send'])):true;
	//获取信息配置
	$conf['co']['title']=(!empty($_POST['title']))?changety(html($_POST['title'])):true;
