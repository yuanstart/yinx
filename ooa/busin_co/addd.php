...
//数据入库
$sql='insert into '.table($conf['sy']['table_co']).' (`lm`,`list_lm`'.$title_key.',`apname`'.$keyword_key.',`link_url`'.$f_body_key.''.$z_body_key.',`img_sl`,`pic_sl`,`fil_sl`,`vid_sl`'.$ym_tit_key.''.$ym_key_key.''.$ym_des_key.',`ding`,`tuijian`,`hot`,`pass`,`read_num`,`px`,`ip`,`wtime`) values('.$lm.',"'.$list_lm.'"'.$title_val.',"'.$apname.'"'.$keyword_val.',"'.$link_url.'"'.$f_body_val.''.rekey($z_body_val).',"'.$img_sl.'","'.$pic_sl.'","'.$fil_sl.'","'.$vid_sl.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',0,0,0,1,0,'.$px.',"'.getip().'",'.$wtime.')';
$db->execute($sql);
$id=$db->insert_id();
