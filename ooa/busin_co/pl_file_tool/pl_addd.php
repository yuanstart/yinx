...
$sql='insert into '.table($conf['pl']['table']).' (`sy_id`,`pl_id`'.$title_key.',`link_url`'.$z_body_key.',`img_sl`,`fil_sl`'.$ym_tit_key.''.$ym_key_key.''.$ym_des_key.',`pass`,`read_num`,`px`,`ip`,`wtime`) values('.$conf['pl']['sy_id'].','.$pr_id.''.$title_val.',"'.$link_url.'"'.$z_body_val.',"'.$img_sl.'","'.$fil_sl.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',1,0,'.$px.',"'.getip().'",'.time().')';
$db->execute($sql);
