...
<input type="button" name="Submit1" value="添加文件" class="btn" onclick="parent.tanchuchuang('pl_file_tool/pl_add.php?pl_id=<?php echo $pl_id?>',820,250);"> 
</div>
<?php
//把图片列出来
if (($pl_id!=''&&checknum($pl_id))||(isset($_SESSION[$conf['pl']['sesname']])&&$_SESSION[$conf['pl']['sesname']]!=''&&checknum($_SESSION[$conf['pl']['sesname']]))){
	if ($pl_id!=''){
		$sql='select id,pl_id,title,fil_sl,px,wtime,read_num,pass from '.table($conf['pl']['table']).' where pl_id='.$pl_id.' and sy_id='.$conf['pl']['sy_id'].' order by px desc,id asc';
	}else{
		$sql='select id,pl_id,title,fil_sl,px,wtime,read_num,pass from '.table($conf['pl']['table']).' where pl_id='.$_SESSION[$conf['pl']['sesname']].' and sy_id='.$conf['pl']['sy_id'].' order by px desc,id asc';
	}
	$rss=$db->getrss($sql);
	if ($rss){
?>
<form id="form1" name="form1" action="pl_make.php?act=px&pl_id=<?php echo $pl_id?>" method="post" >
    ...
    <tr class="title">
      ...
      <tr class="tdbg">
        ...
        <td align="center">
        <a href="javascript:;" onclick="parent.tanchuchuang('pl_file_tool/pl_edit.php?id=<?php echo $row['id']?>&pl_id=<?php echo $row['pl_id']?>',820,250);">修改</a> | <a href="pl_make.php?id=<?php echo $row['id']?>&pl_id=<?php echo $pl_id?>&act=del"  onClick="return confirm(\'确定要删除该数据吗?\')" >删除</a>
        </td>
      </tr>
        <?php
            $a++;
