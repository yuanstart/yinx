...
function subm(str){
	if (checkForm('form1')){
		if (str==1){
			gt('form1').action='addd.php?act=1';
		}else if(str==2){
			if (gt('r_sel')==null&&gt('r_sel_email').value==''){
				alert('请填写收件箱');
				return false;
			}
			gt('form1').action='addd.php?act=2';
		}
		gt('form1').submit();
	}
}
...
<br />
<FORM name="form1" id="form1" method="post" action="addd.php" onSubmit="return checkForm('form1')">
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
  <tr class="title">
    <td colspan="2">&nbsp;</td>
  </tr>
  <tr>
    ...
    <div class="clear"></div>
    <div class="tj_list"></div>
    <textarea name="r_sel_email" rows="4" id="r_sel_email" style="width:665px;clear:both;"></textarea>
    多个邮箱用英文的","隔开
    </div>
    </td>
  </tr>
  <tr>
    ...
    <td class="tdbg">
    <input name="title" id="title" type="text" class="input_m" size="70" maxlength="100" canEmpty="N" checkType="string,," checkStr="邮件标题" />
  ...
  <tr class="tdbg">
    ...
    <td>
    <ul class="mb_list">
    	<li lin="" class="cur">不使用模板</li>
    	<?php 
		$rss=$db->getrss('select id,bname from '.table('email_mb').' where pass=1 order by px desc,id desc');
		foreach($rss as $v){
		?>
    	<li lin="<?php echo $v['id']?>"><?php echo $v['bname']?></li>
        <?php
        }
		?>
        <div class="clear"></div>
    </ul>
    <textarea id="z_body" name="z_body" style="width:670px;height:300px;display:none;"  canEmpty="N" checkType="string,," checkStr="邮件内容"></textarea>
    </td>
  </tr>
  ...
  <script>
        ...
        KindEditor.ready(function(K) {
			...
			//3.然后改一下文本区域的名字
			ke=K.create('textarea[name="z_body"]',options);
			
        });
		
		$('.mb_list li').click(function(){
			if (confirm('确定要使用该模板吗？目前的邮件内容将不会保存起来的')){
				$('.mb_list li').removeClass('cur');
				$(this).addClass('cur');
				if($(this).attr('lin')!=''){
					$.getJSON('ajax_mb.php', 
						{'id':$(this).attr('lin')}, 
  ...
  <tr>
    ...
    <td class="tdbg">
		...
		foreach($rss as $v){
		?>
            <li title="<?php echo $v['email'];?>"><input type="checkbox" class="checkbox er" name="s_email[]" value="<?php echo $v['id'];?>" /><?php echo getstr($v['email'],19);?></li>
		<?php
		}
		?>
        </ul>
  ...
  <tr>
    ...
    <td class="tdbg">
    <input name="s_name" type="text" class="input_m" size="30" maxlength="50"  canEmpty="N" checkType="string,," checkStr="发件称呼" /> <span class="red">*</span>
    </td>
  </tr>
  <tr>
    <td width="120" align="right" class="tdbg">每次发送：</td>
    <td class="tdbg">
    <input name="s_num" type="text" class="input_m" size="5" maxlength="3" value="10"  canEmpty="N" checkType="number,," checkStr="每次发送多少封邮件" /> 
    封 发送间隔
    <input name="s_time" type="text" class="input_m" size="5" maxlength="3" value="10"  canEmpty="N" checkType="number,," checkStr="发送间隔" /> 
    ...
    <td class="tdbg">
    <input name="h_email" type="text" class="input_m" size="30" maxlength="50"  canEmpty="N" checkType="string,," checkStr="回复邮箱" /> <span class="red">*</span>
    </td>
  </tr>
</table>
