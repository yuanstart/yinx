...
if ($id==''||!checknum($id)||$s_email==''||$s_name==''||$s_num==''||!checknum($s_num)||$s_time==''||!checknum($s_time)||$h_email==''||$s_email==''||$title==''){
	msg('参数错误!');
}
...
//更新入库
$sql='update '.table('email_co').'  set `r_sel`="'.$r_sel.'",`r_sel_stime`="'.$r_sel_stime.'",`r_sel_etime`="'.$r_sel_etime.'",`r_sel_email`="'.$r_sel_email.'",`r_email`="'.$r_email.'",`s_email`="'.$s_email.'",`s_name`="'.$s_name.'",`s_num`="'.$s_num.'",`s_time`="'.$s_time.'",`h_email`="'.$h_email.'",`title`="'.$title.'",`z_body`="'.$z_body.'",`fil_sl`="'.$fil_sl.'" where `id`='.$id.'';
$db->execute($sql);
