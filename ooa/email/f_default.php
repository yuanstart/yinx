...
<br />
<form id="sform" name="sform" method="get" action="f_default.php">
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
  <tr class="tdbg3">
...
<br />
<form id="form1" name="form1" action="f_make.php" method="post" >
...
<?php
$sql='select * from '.table('email_fj').' where 1=1 '.$sq.' order by `id` desc';
...
?>
    <tr class="tdbg">
        ...
        <?php 
		echo ($row['pass']==1)?'<a class="icon b" href="f_make.php?id='.$row["id"].'&ac=pass2" title="点击屏蔽"></a>':'<a class="icon bn" href="f_make.php?id='.$row["id"].'&ac=pass1" title="点击取消屏蔽"></a>';
		?>
        </td></tr></table>
        </td>
        <td align="center">
        <a href="f_edit.php?id=<?php echo $row['id']?>">修改</a> | 
        <a href="f_make.php?id=<?php echo $row['id']?>&ac=del"  onClick="return confirm('确定要删除该数据吗?')">删除</a>
        </td>
    </tr>
<?php
}
