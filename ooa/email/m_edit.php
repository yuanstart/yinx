...
$url=(previous())?previous():'m_default.php';

if ($id==''||!checknum($id)){
	msg('参数错误！');
}

$row=$db->getrs('select * from '.table('email_mb').' where `id`='.$id.'');
if (!$row){
	msg('该信息不存在或已删除');
}
?>
...
<br />
<FORM id="form1" name="form1" onSubmit="return checkForm('form1')" action="m_editt.php" method="post">
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
  ...
  <tr>
    ...
    <td class="tdbg">
    <input name="bname" type="text" class="input_m" size="30" maxlength="50" canEmpty="N" checkType="string,," checkStr="模板名称" value="<?php echo $row['bname']?>" />
    <span class="red">*</span>
    </td>
  </tr>
  <tr>
    ...
    <td class="tdbg">
    <input name="title" type="text" class="input_m" size="45" maxlength="100" canEmpty="N" checkType="string,," checkStr="邮件标题" value="<?php echo $row['title']?>" />
    <span class="red">*</span>
    </td>
  </tr>
  <tr class="tdbg">
