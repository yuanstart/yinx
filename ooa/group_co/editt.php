...
$title='';
$title_val='';$keyword_val='';$info_from_val='';$info_author_val='';$f_body_val='';$z_body_val='';
...
foreach($cong['mlang'] as $k=>$v){
	...
	$info_from_val .=',`info_from'.$v['lang'].'`="'.(!empty($_POST['info_from'.$v['lang']])?html($_POST['info_from'.$v['lang']]):'').'"';
	$info_author_val .=',`info_author'.$v['lang'].'`="'.(!empty($_POST['info_author'.$v['lang']])?html($_POST['info_author'.$v['lang']]):'').'"';
...
$sql='update '.table($conf['sy']['table_co']).' set `lm`='.$lm.',`list_lm`="'.$list_lm.'"'.$title_val.',`apname`="'.$apname.'"'.$keyword_val.',`link_url`="'.$link_url.'"'.$info_from_val.''.$info_author_val.''.$f_body_val.''.rekey($z_body_val).',`img_sl`="'.$img_sl.'",`pic_sl`="'.$pic_sl.'",`fil_sl`="'.$fil_sl.'",`vid_sl`="'.$vid_sl.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',`px`='.$px.',`wtime`='.$wtime.' where `id`='.$id.'';
$db->execute($sql);
