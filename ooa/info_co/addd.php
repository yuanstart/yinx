...
$title='';
$title_key='';$title_val='';$keyword_key='';$keyword_val='';$info_from_key='';$info_from_val='';$info_author_key='';$info_author_val='';$f_body_key='';$f_body_val='';$z_body_key='';$z_body_val='';
...
foreach($cong['mlang'] as $k=>$v){
	...
	$info_from_key.=',`info_from'.$v['lang'].'`';
	$info_from_val.=',"'.(!empty($_POST['info_from'.$v['lang']])?html($_POST['info_from'.$v['lang']]):'').'"';
	$info_author_key.=',`info_author'.$v['lang'].'`';
	$info_author_val.=',"'.(!empty($_POST['info_author'.$v['lang']])?html($_POST['info_author'.$v['lang']]):'').'"';
...
//数据入库
$sql='insert into '.table($conf['sy']['table_co']).' (`lm`,`list_lm`'.$title_key.',`apname`'.$keyword_key.',`link_url`'.$info_from_key.''.$info_author_key.''.$f_body_key.''.$z_body_key.',`img_sl`,`pic_sl`,`fil_sl`,`vid_sl`'.$ym_tit_key.''.$ym_key_key.''.$ym_des_key.',`ding`,`tuijian`,`hot`,`pass`,`read_num`,`px`,`ip`,`wtime`) values('.$lm.',"'.$list_lm.'"'.$title_val.',"'.$apname.'"'.$keyword_val.',"'.$link_url.'"'.$info_from_val.''.$info_author_val.''.$f_body_val.''.rekey($z_body_val).',"'.$img_sl.'","'.$pic_sl.'","'.$fil_sl.'","'.$vid_sl.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',0,0,0,1,0,'.$px.',"'.getip().'",'.$wtime.')';
$db->execute($sql);
$id=$db->insert_id();
