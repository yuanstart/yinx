...
if($ac=='px'){
	...
	//删除
		...
		foreach($rsk as $rsm){
			...
			foreach ($rsn as $rst){
				$conf_um=unserialize($rst['info_img_sm']);
				$conf_up=unserialize($rst['info_pic_sm']);
				...
				foreach($rss as $row){
					...
					if(!empty($conf_pi)){
						//删除相关信息的图片
						$sql='select `id`,`img_sl` from '.table($conf_pi['pl']['table']).' where `sy_id`='.$conf_pi['pl']['sy_id'].' and `pl_id`='.$row['id'].'';
						$arr=$db->getrss($sql);
						foreach($arr as $rsb){
							if($rsb['img_sl']!=''){
						...
						//删除相关信息的记录
						$db->execute('delete from '.table($conf_pi['pl']['table']).' where `sy_id`='.$conf_pi['pl']['sy_id'].' and `pl_id`='.$row['id'].'');
					}
					//删除相关文件
					if(!empty($conf_pf)){
						//删除相关文件的图片、文件
						$sql='select `id`,`img_sl`,`fil_sl` from '.table($conf_pf['pl']['table']).' where `sy_id`='.$conf_pf['pl']['sy_id'].' and `pl_id`='.$row['id'].'';
						$arr=$db->getrss($sql);
						foreach($arr as $rsb){
							if($rsb['img_sl']!=''){
						...
						//删除相关文件的记录
						$db->execute('delete from '.table($conf_pf['pl']['table']).' where `sy_id`='.$conf_pf['pl']['sy_id'].' and `pl_id`='.$row['id'].'');
					}
					//删除相关视频
					if(!empty($conf_pv)){
						//删除相关视频的图片、视频
						$sql='select `id`,`img_sl`,`vid_sl` from '.table($conf_pv['pl']['table']).' where `sy_id`='.$conf_pv['pl']['sy_id'].' and `pl_id`='.$row['id'].'';
						$arr=$db->getrss($sql);
						foreach($arr as $rsb){
							if($rsb['img_sl']!=''){
						...
						//删除相关视频的记录
						$db->execute('delete from '.table($conf_pv['pl']['table']).' where `sy_id`='.$conf_pv['pl']['sy_id'].' and `pl_id`='.$row['id'].'');
					}
					//删除组图
					if(!empty($conf_pz)){
						//删除分组的图片
