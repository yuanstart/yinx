...
$title='';
$title_key='';$title_val='';$address_key='';$address_val='';$f_body_key='';$f_body_val='';$z_body_key='';$z_body_val='';
...
foreach($cong['mlang'] as $k=>$v){
	...
	if ($conf['co']['address']==true){
		$address_key.=',`address'.$v['lang'].'`';
		$address_val.=',"'.(!empty($_POST['address'.$v['lang']])?html($_POST['address'.$v['lang']]):'').'"';
	}
	if ($conf['co']['f_body']==true){
		$f_body_key.=',`f_body'.$v['lang'].'`';
		$f_body_val.=',"'.(!empty($_POST['f_body'.$v['lang']])?$_POST['f_body'.$v['lang']]:'').'"';
	}
...
//数据入库
$sql='insert into '.table($conf['sy']['table_co']).' (`lm`,`list_lm`'.$title_key.',`apname`,`link_url`,`num`'.$address_key.',`stime`,`etime`'.$f_body_key.''.$z_body_key.''.$ym_tit_key.''.$ym_key_key.''.$ym_des_key.',`ding`,`tuijian`,`hot`,`pass`,`read_num`,`px`,`ip`,`wtime`) values('.$lm.',"'.$list_lm.'"'.$title_val.',"'.$apname.'","'.$link_url.'","'.$num.'"'.$address_val.',"'.$stime.'","'.$etime.'"'.$f_body_val.''.rekey($z_body_val).''.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',0,0,0,1,0,'.$px.',"'.getip().'",'.$wtime.')';
$db->execute($sql);
$id=$db->insert_id();
