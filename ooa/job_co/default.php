...
<?php
$sql='select a.`id`,a.`lm`,a.`title`,a.`ding`,a.`tuijian`,a.`hot`,a.`pass`,a.`read_num`,a.`px`,a.`wtime`,b.`title_lm`,b.`add_xx` from '.table($conf['sy']['table_co']).' a left join '.table($conf['sy']['table_lm']).' b on a.`lm`=b.`id_lm` where 1=1 '.$sq.' order by a.ding desc,a.`px` desc,a.`id` desc';
...
?>
    <tr class="tdbg">
		...
		?>
        <td align="center">
        <a href="yp_default.php?keyword=<?php echo $row['title']?>&p_url=<?php echo urlencode(getpageurl());?>">查看应聘</a> | 
