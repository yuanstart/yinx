...
$title='';
$title_val='';$address_val='';$f_body_val='';$z_body_val='';
...
foreach($cong['mlang'] as $k=>$v){
	...
	if ($conf['co']['address']==true){
		$address_val .=',`address'.$v['lang'].'`="'.(!empty($_POST['address'.$v['lang']])?html($_POST['address'.$v['lang']]):'').'"';
	}
	if ($conf['co']['f_body']==true){
		$f_body_val  .=',`f_body'.$v['lang'].'`="'.(!empty($_POST['f_body'.$v['lang']])?$_POST['f_body'.$v['lang']]:'').'"';
	}
...
$sql='update '.table($conf['sy']['table_co']).' set `lm`='.$lm.',`list_lm`="'.$list_lm.'"'.$title_val.',`apname`="'.$apname.'",`link_url`="'.$link_url.'",`num`="'.$num.'"'.$address_val.',`stime`="'.$stime.'",`etime`="'.$etime.'"'.$f_body_val.''.rekey($z_body_val).''.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',`px`='.$px.',`wtime`='.$wtime.' where `id`='.$id.'';
$db->execute($sql);
