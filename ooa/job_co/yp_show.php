...
$row=$db->getrs('select * from '.table($conf['sy']['table_yp']).' where `id`='.$id.'');
if (!$row){
	msg('该招聘不存在或已删除');
}
$db->execute('update '.table($conf['sy']['table_yp']).' set chakan=1 where `id`='.$id.'');
?>
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
  ...
  <tr class="tdbg">
    ...
    <td><textarea name="textarea" cols="70" rows="6"><?php echo rehtml($row['z_body'])?></textarea></td>
  </tr>
     <tr class="tdbg">
