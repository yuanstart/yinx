...
$title_lm='';
$title_lm_key='';$title_lm_val='';$z_body_lm_key='';$z_body_lm_val='';
...
//添加分类信息
$sql='insert into '.table($conf['sy']['table_lm']).' (`fid`'.$title_lm_key.',`apname_lm`,`url_lm`'.$z_body_lm_key.',`add_xx`,`add_xia`,`con_att`'.$ym_tit_key.''.$ym_key_key.''.$ym_des_key.',`tuijian`,`hot`,`pass`,`px`,`wtime`,`ip`) values('.$fid.''.$title_lm_val.',"'.$apname_lm.'","'.$url_lm.'"'.$z_body_lm_val.',"'.$add_xx.'","'.$add_xia.'","'.$con_att.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',0,0,1,'.$px.','.time().',"'.getip().'")';
$db->execute($sql);
