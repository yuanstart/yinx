...
$sql='update '.table($conf['sy']['table_lm']).' set `fid`='.$fid.''.$title_lm_val.',`apname_lm`="'.$apname_lm.'",`url_lm`="'.$url_lm.'"'.$z_body_lm_val.',`add_xx`="'.$add_xx.'",`add_xia`="'.$add_xia.'",`con_att`="'.$con_att.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',`px`='.$px.' where `id_lm`='.$id.'';
$db->execute($sql);
