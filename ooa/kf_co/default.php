      		...
      		<td <?php if ($conf['sy']['need_lx']==false){echo ' style="display:none;"';}?>>
                <select name="lx" id="lx" onchange="location='default.php?lx='+this.value+'&lm=<?php echo $lm?>&zt_val=<?php echo $zt_val?>&keyword=<?php echo $keyword?>'">
                  <option value="" selected="selected">请选择类型</option>
                    <?php
                    $rss=$db->getrss('select * from '.table($conf['sy']['table_lx']).' order by `px` desc,`id_lm` asc');
                    foreach($rss as $rs){
...
<?php
$sql='select a.`id`,a.`lm`,a.`title`,a.`ding`,a.`tuijian`,a.`hot`,a.`pass`,a.`px`,b.`title_lm` from '.table($conf['sy']['table_co']).' a left join '.table($conf['sy']['table_lx']).' b on a.`lx`=b.`id_lm` where 1=1 '.$sq.' order by a.ding desc,a.`px` desc,a.`id` desc';
