...
$title='';
$title_key='';$title_val='';$link_url_key='';$link_url_val='';$f_body_key='';$f_body_val='';$z_body_key='';$z_body_val='';$img_sl_key='';$img_sl_val='';
...
foreach($cong['mlang'] as $k=>$v){
	...
	if ($conf['co']['img_sl']==true){
		$img_sl_key.=',`img_sl'.$v['lang'].'`';
		$img_sl_val.=',"'.(!empty($_POST['img_sl'.$v['lang']])?html($_POST['img_sl'.$v['lang']]):'').'"';
	}
...
//数据入库
$sql='insert into '.table($conf['sy']['table_co']).' (`lm`,`list_lm`'.$title_key.',`apname`'.$link_url_key.''.$f_body_key.''.$z_body_key.''.$img_sl_key.''.$ym_tit_key.''.$ym_key_key.''.$ym_des_key.',`ding`,`tuijian`,`hot`,`pass`,`read_num`,`px`,`ip`,`wtime`) values('.$lm.',"'.$list_lm.'"'.$title_val.',"'.$apname.'"'.$link_url_val.''.$f_body_val.''.rekey($z_body_val).''.$img_sl_val.''.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',0,0,0,1,0,'.$px.',"'.getip().'",'.$wtime.')';
$db->execute($sql);
$id=$db->insert_id();
