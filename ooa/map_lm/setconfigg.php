...
if (read_config('config')){
	//获取系统配置
	$sy_id=$conf['sy']['id']=(!empty($_POST['id']))?(int)html($_POST['id']):2;
	$conf['sy']['pre']=(!empty($_POST['pre']))?html($_POST['pre']):'news';
	$conf['sy']['name']=(!empty($_POST['name']))?html($_POST['name']):'新闻';
	$conf['sy']['table_lm']=(!empty($_POST['table_lm']))?html($_POST['table_lm']):'news_lm';
	$conf['sy']['table_co']=(!empty($_POST['table_co']))?html($_POST['table_co']):'news_co';
	...
	$conf['co']['phone']=(!empty($_POST['phone']))?changety(html($_POST['phone'])):false;
	$conf['co']['address']=(!empty($_POST['address']))?changety(html($_POST['address'])):false;
	$conf['co']['zuobiao']=(!empty($_POST['zuobiao']))?changety(html($_POST['zuobiao'])):false;
