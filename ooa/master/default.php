...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border listtable">
  <tr class="title">
    ...
    <td width="150" align="center">最后登录时间</td>
    <td width="70" align="center">状态</td>
    <td width="200" align="center">管理操作</td>
  </tr>
<?php
$sql='select * from '.table('master').' where 1=1 '.$sq.' order by `id` desc';
...
?>
    <tr class="tdbg">
        ...
        <?php 
		echo ($row['pass']==1)?'<a class="icon b" href="make.php?id='.$row["id"].'&ac=pass2" title="点击成为屏蔽"></a>':'<a class="icon bn" href="make.php?id='.$row["id"].'&ac=pass1" title="点击取消屏蔽"></a>';
		?>
        </td></tr></table>
        </td>
        <td align="center">
        <a href="default_log.php?username=<?php echo $row['username']?>&p_url=<?php echo urlencode(getpageurl());?>">查看日志</a> | 
