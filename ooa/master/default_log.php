...
//如果有关键字
if ($keyword!=''){
	$sq.=' and (username like "%'.$keyword.'%" or `ip` like "%'.$keyword.'%" or `z_body` like "%'.$keyword.'%")';
}
?>
...
<br />
<form id="sform" name="sform" method="get" action="default_log.php">
    ...
    <td>
        <table border="0" cellspacing="0" cellpadding="0" style="float:left;">
          <tr>
            ...
            <td>
            <select name="username" id="username" onchange="gt('sform').submit()">
            <option value="">管理员</option>
            <?php
            $rows=$db->getrss('select username from master  order by id desc');
			foreach($rows as $row){
			...
			?>
			 ...
			 <script language="javascript">
                gt("username").value="<?php echo $username?>";
             </script>
            </td>
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border listtable">
  <tr class="title">
    ...
    <td align="center" width="120">用户名</td>
    <td align="left" style="padding-left:5px;">操作</td>
  </tr>
<?php
$sql='select * from '.table('master_log').' where 1=1 '.$sq.' order by `id` desc';
...
?>
    <tr class="tdbg">
        ...
        <td align="left" style="padding-left:5px;"><?php echo $row['z_body']?></td>
    </tr>
<?php
}
?>
