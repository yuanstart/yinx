  ...
  <tr class="tdbg3">
    ...
    <td>
        <table border="0" cellspacing="2" cellpadding="0" style="float:left;">
          <tr>
            <td>从&nbsp;<input name="s_wtime" type="text" id="s_wtime" size="10" maxlength="50"  onclick="return Calendar('s_wtime');" value="<?php echo $s_wtime?>" placeholder="注册日期" />
            &nbsp;到&nbsp;<input name="e_wtime" type="text" id="e_wtime" size="10" maxlength="50"  onclick="return Calendar('e_wtime');" value="<?php echo $e_wtime?>" placeholder="注册日期" /></td>
            <td>从&nbsp;<input name="s_etime" type="text" id="s_etime" size="10" maxlength="50"  onclick="return Calendar('s_etime');" value="<?php echo $s_etime?>" placeholder="登录日期"  />
            &nbsp;到&nbsp;<input name="e_etime" type="text" id="e_etime" size="10" maxlength="50"  onclick="return Calendar('e_etime');" value="<?php echo $e_etime?>" placeholder="登录日期" /></td>
...
<table width="100%" border="0" cellspacing="1" cellpadding="2" class="border">
  <tr>
    <td class="tdbgo">
    <?php
    $sql='select count(*) as acount,sum(if(pass=1,1,0)) as pass1,sum(if(pass=2,1,0)) as pass2,sum(if(pass=3,1,0)) as pass3,sum(if(pass=4,1,0)) as pass4 from '.table($conf['sy']['table_co']).' where 1=1 '.$sq;
    $rs=$db->getrs($sql);
    if ($rs){
	?>
		总个数：<span style="color:#ff0000;font-weight:bold;"><?php echo $rs['acount']?></span>
...
<?php
$sql='select `id`,`username`,`rename`,`wtime`,`etime`,`login_num`,`pass` from '.table($conf['sy']['table_co']).' where 1=1 '.$sq.' order by `id` desc';
...
?>
    <tr class="tdbg">
        ...
        <td align="center"><?php echo ($row['etime']>0)?date('Y-m-d H:i:s',$row['etime']):'N/A';?></td>
