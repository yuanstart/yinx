...
$title='';
$title_val='';$keyword_val='';$f_body_val='';$z_body_val='';$t_body_val='';$g_body_val='';
$pro_can1_val='';$pro_can2_val='';$pro_can3_val='';$pro_can4_val='';
...
foreach($cong['mlang'] as $k=>$v){
	...
	if ($conf['co']['pro_can1']==true){
		$pro_can1_val .=',`pro_can1'.$v['lang'].'`="'.(!empty($_POST['pro_can1'.$v['lang']])?html($_POST['pro_can1'.$v['lang']]):'').'"';
	}
	if ($conf['co']['pro_can2']==true){
		$pro_can2_val .=',`pro_can2'.$v['lang'].'`="'.(!empty($_POST['pro_can2'.$v['lang']])?html($_POST['pro_can2'.$v['lang']]):'').'"';
	}
	if ($conf['co']['pro_can3']==true){
		$pro_can3_val .=',`pro_can3'.$v['lang'].'`="'.(!empty($_POST['pro_can3'.$v['lang']])?html($_POST['pro_can3'.$v['lang']]):'').'"';
	}
	if ($conf['co']['pro_can4']==true){
		$pro_can4_val .=',`pro_can4'.$v['lang'].'`="'.(!empty($_POST['pro_can4'.$v['lang']])?html($_POST['pro_can4'.$v['lang']]):'').'"';
	}
	...
	if ($conf['co']['t_body']==true){
		$t_body_val  .=',`t_body'.$v['lang'].'`="'.(!empty($_POST['t_body'.$v['lang']])?$_POST['t_body'.$v['lang']]:'').'"';
	}
	if ($conf['co']['g_body']==true){
		$g_body_val  .=',`g_body'.$v['lang'].'`="'.(!empty($_POST['g_body'.$v['lang']])?$_POST['g_body'.$v['lang']]:'').'"';
	}
...
$sql='update '.table($conf['sy']['table_co']).' set `lm`='.$lm.',`list_lm`="'.$list_lm.'"'.$title_val.',`apname`="'.$apname.'"'.$keyword_val.',`link_url`="'.$link_url.'"'.$pro_can1_val.''.$pro_can2_val.''.$pro_can3_val.''.$pro_can4_val.''.$f_body_val.''.rekey($z_body_val).''.rekey($t_body_val).''.rekey($g_body_val).',`img_sl`="'.$img_sl.'",`pic_sl`="'.$pic_sl.'",`fil_sl`="'.$fil_sl.'",`vid_sl`="'.$vid_sl.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',`px`='.$px.',`wtime`='.$wtime.' where `id`='.$id.'';
$db->execute($sql);
