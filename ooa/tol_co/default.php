...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
  <tr class="tdbg3">
    <td width="70" align="right"><strong>信息搜索：</strong></td>
    <td>
...
<?php
$sql='select a.`id`,a.`lm`,a.`title`,a.`img_sl`,a.`con_att`,a.`ding`,a.`tuijian`,a.`hot`,a.`pass`,a.`read_num`,a.`px`,b.`title_lm`,b.`add_xx` from '.table($conf['sy']['table_co']).' a left join '.table($conf['sy']['table_lm']).' b on a.`lm`=b.`id_lm` where 1=1 '.$sq.' order by a.ding desc,a.`px` desc,a.`id` asc';
...
?>
    <tr class="tdbg">
		...
		<?php 
		$del_a=($row['add_xx']=='no'||$row['con_att']==2)?'<span class="hui"> | 删除</span>':' | <a href="make.php?id='.$row['id'].'&ac=del"  onClick="return confirm(\'确定要删除该数据吗?\')" >删除</a>';
...
<?php 
if ($conf['co']['add_xx']==true){
?>
&nbsp;<input name="" type="button"  class="btn" value="删除选中" onclick="act('form1','del');"/>
<?php 
}
?>
<?php 
if ($conf['co']['ding']==true){
?>
&nbsp;<input name="" type="button"  class="btn" value="置顶选中" onclick="act('form1','ding1');"/>
&nbsp;<input name="" type="button"  class="btnn" value="取消置顶" onclick="act('form1','ding2');"/>
<?php 
}
?>
<?php 
if ($conf['co']['tuijian']==true){
?>
&nbsp;<input name="" type="button"  class="btn" value="推荐选中" onclick="act('form1','tuijian1');"/>
&nbsp;<input name="" type="button"  class="btnn" value="取消推荐" onclick="act('form1','tuijian2');"/>
<?php 
}
?>
<?php 
if ($conf['co']['hot']==true){
?>
&nbsp;<input name="" type="button"  class="btn" value="热点选中" onclick="act('form1','hot1');"/>
&nbsp;<input name="" type="button"  class="btnn" value="取消热点" onclick="act('form1','hot2');"/>
<?php 
}
?>
<?php 
if ($conf['co']['pass']==true){
?>
&nbsp;<input name="" type="button"  class="btn" value="屏蔽选中" onclick="act('form1','pass2');"/>
&nbsp;<input name="" type="button"  class="btnn" value="取消屏蔽" onclick="act('form1','pass1');"/>
<?php 
}
?>
</p>
