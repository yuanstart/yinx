...
$sql='update '.table($conf['sy']['table_co']).' set `lm`='.$lm.',`list_lm`="'.$list_lm.'"'.$title_val.',`apname`="'.$apname.'"'.$keyword_val.',`link_url`="'.$link_url.'"'.$f_body_val.''.rekey($z_body_val).',`img_sl`="'.$img_sl.'"'.$ym_tit_val.''.$ym_key_val.''.$ym_des_val.',`px`='.$px.',`wtime`='.$wtime.',`con_att`='.$con_att.' where `id`='.$id.'';
$db->execute($sql);
