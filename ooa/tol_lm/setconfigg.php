...
if (read_config('config')){
	//获取系统配置
	$sy_id=$conf['sy']['id']=(!empty($_POST['id']))?(int)html($_POST['id']):1;
	$conf['sy']['pre']=(!empty($_POST['pre']))?html($_POST['pre']):'tol';
	$conf['sy']['name']=(!empty($_POST['name']))?html($_POST['name']):'资料';
	$conf['sy']['table_lm']=(!empty($_POST['table_lm']))?html($_POST['table_lm']):'tol_lm';
	$conf['sy']['table_co']=(!empty($_POST['table_co']))?html($_POST['table_co']):'tol_co';
	...
	$conf['co']['con_att']=(!empty($_POST['con_att']))?changety(html($_POST['con_att'])):false;
